...
# replace with path to your copy of "hvac" nodejs tuya CLI
node_hvac="/absolute-path-to/hvac"
...
if (( ${#current[@]} )); then
  ...
  # calculate rolling averages from last 7 measurements
  avg_t=$(cat /private/var/log/hvac/temp-humid.log | tail -n 7 | awk '{ sum += $2; n++} END {if (n > 0) print sum / n; }')
  avg_h=$(cat /private/var/log/hvac/temp-humid.log | tail -n 7 | awk '{ sum += $3; n++} END {if (n > 0) print sum / n; }')

  # log latest measurements
  echo -n "$(date '+%Y_%m_%d-%H%M') ${current[1]} ${current[2]} ${avg_t} ${avg_h}" >> /private/var/log/hvac/temp-humid.log

  # turn heater on or off based on temperature preferences and log event or lack thereof
  if [[ ${avg_t} -gt ${max_t} ]] ; then
    /usr/local/bin/node "${node_hvac}/bin/index.js" office-heater off
    echo " off" >> /private/var/log/hvac/temp-humid.log
  elif [[ ${avg_t} -lt ${min_t} ]] ; then
    /usr/local/bin/node "${node_hvac}/bin/index.js" office-heater on
    echo " on" >> /private/var/log/hvac/temp-humid.log
  else
    echo " --" >> /private/var/log/hvac/temp-humid.log
  fi
